...
import { PressFormTZPScreenItem } from '../components/componentsLists/pressFormTZPScreenItem';
import { TZPPlateScreenItems } from '../components/componentsLists/TZPPlateScreenItems';

type TTZPPlate = {
  route: { name: string };
}

export const TZPPlateScreen: React.FC<TTZPPlate> = (props) => {
  const { data, updateResponse, isLoading, error } = UseHttpLatest<IHydraGet<TZPPlate>>();
    ...
    <View style={Cstyles.containerc}>
      ...
      <View style={{height: '87%'}}>
        ...
        <View style={{marginTop: -410}}>
          ...
          { isLoading ? (<Loader/>) 
            : (data?.['hydra:member'] && <TZPPlateScreenItems data={data?.['hydra:member']} />) 
          }
