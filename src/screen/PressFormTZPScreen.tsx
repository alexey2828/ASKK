...
export const PressFormTZPScreen: React.FC<TTZPPressDetail> = (props) => {

  const { data, updateResponse, isLoading, error } = UseHttpLatest<IHydraGet<ITZPPressDetail>>();
    ...
    <View style={Cstyles.containerc}>
      ...
      <View style={{height: '87%'}}>
        ...
        <View style={{marginTop: -410}}>
          ...
          { isLoading ? (<Loader/>) 
            : (data?.['hydra:member'] && <PressFormTZPScreenItem data={data?.['hydra:member']} />) 
          }
